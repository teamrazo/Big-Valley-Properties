...
export default function Step1TypeAddress({ data, update, onNext }: StepProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input id="city" type="text" value={data.city} onChange={e => update({ city: e.target.value })}
            className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-charcoal-ink dark:text-white focus:ring-2 focus:ring-forest-green focus:border-transparent outline-none transition min-h-[48px]"
        ...
        <div>
          ...
          <input id="postalCode" type="text" value={data.postalCode} onChange={e => update({ postalCode: e.target.value })}
            className="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-charcoal-ink dark:text-white focus:ring-2 focus:ring-forest-green focus:border-transparent outline-none transition min-h-[48px]"
            placeholder="96093" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-3">
          {COUNTIES.map(county => (
            <button key={county} type="button" onClick={() => update({ county })}
