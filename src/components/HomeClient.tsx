  ...
  return (
    <>
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden">
        {/* Static image fallback (always visible, serves as mobile bg + video poster) */}
        <ParallaxImage
        ...
        <div className="container-bvp relative z-10 py-20" style={{ zIndex: 2 }}>
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="max-w-2xl">
              ...
              >
                <Link href="/properties" className="btn-primary btn-glow btn-magnetic">
                  Browse Properties
                </Link>
                <Link
                  href="/contact"
      ...
      {/* ═══════════════════════════════════════════════
          2. BROKERAGE
      ═══════════════════════════════════════════════ */}
      <section className="section-padding bg-warm-cream dark:bg-gray-950 overflow-hidden">
        <div className="container-bvp">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <MotionWrapper variant="fadeRight" delay={0.1}>
              ...
              <p className="text-cabin-timber dark:text-gray-400 leading-relaxed mb-8">
      ...
      ═══════════════════════════════════════════════ */}
      <section className="section-padding bg-luxury-light dark:bg-gray-900">
        <div className="container-bvp">
        ...
        <div className="container-bvp">
          <MotionWrapper className="text-center mb-12">
            <div className="flex items-center justify-center gap-2 mb-3">
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 text-xs rounded-full font-medium">
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                AI-Curated
              </span>
              <p className="text-forest-green dark:text-river-stone font-body text-sm uppercase tracking-brand-widest">
                Handpicked For You
              </p>
            </div>
          ...
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {allAgents.map((agent) => (
              <motion.div key={agent.id} variants={staggerItem}>
                <Link href={`/agents/${agent.slug}`} className="group block team-card bg-white dark:bg-gray-800 rounded-lg overflow-hidden border border-gray-100 dark:border-gray-700 shadow-luxury-sm">
                  {/* Agent Photo */}
                  ...
                  <div className="p-5">
                    <h3 className="font-heading text-lg text-charcoal-ink dark:text-white group-hover:text-forest-green dark:group-hover:text-river-stone transition-colors duration-300">
                      {agent.name}
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {agent.counties.map((county) => (
                        <span
                          key={county}
                          className="px-2.5 py-0.5 bg-canvas-sand dark:bg-gray-700 text-cabin-timber dark:text-gray-300 text-xs rounded"
                        >
                          {county} County
                        </span>
                      ))}
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                      ...
                      <span className="text-forest-green dark:text-river-stone text-xs font-medium uppercase tracking-wide group-hover:translate-x-1 transition-transform duration-300">
                        Profile →
                      </span>
                    </div>
                  </div>
        ...
        <div className="container-bvp">
          <MotionWrapper className="text-center mb-14">
            ...
            <div className="flex items-center justify-center gap-3 mt-4">
              ...
              <span className="font-heading text-xl text-charcoal-ink dark:text-white">4.9</span>
              <span className="text-alpine-slate dark:text-gray-400 text-sm">out of 5 stars from 50+ reviews</span>
            </div>
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <span className="text-alpine-slate dark:text-gray-500 text-xs">Google Reviews</span>
              <span className="text-alpine-slate dark:text-gray-500 text-xs mx-1">•</span>
              <span className="text-alpine-slate dark:text-gray-500 text-xs">100% Customer Satisfaction</span>
            </div>
          </MotionWrapper>
          ...
          <StaggerContainer className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {googleReviews.map((review) => (
              <motion.div
                key={review.name}
                variants={staggerItem}
                className="p-6 bg-white dark:bg-gray-900 rounded-lg testimonial-card border border-gray-100 dark:border-gray-800 shadow-luxury-sm"
              >
                <div className="flex items-center justify-between mb-4">
                  <StarRating rating={review.rating} />
                  <span className="text-xs text-alpine-slate dark:text-gray-500">{review.date}</span>
                </div>
                <p className="font-accent italic text-charcoal-ink dark:text-gray-200 leading-relaxed mb-5">
                  &ldquo;{review.text}&rdquo;
                </p>
                <div className="flex items-center gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
                  <div className="w-10 h-10 rounded-full bg-forest-green/10 dark:bg-forest-green/20 flex items-center justify-center">
                    <span className="text-forest-green dark:text-river-stone font-heading text-sm">
                      {review.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
                    </span>
                  </div>
                  <div>
                    <p className="font-medium text-charcoal-ink dark:text-white text-sm">{review.name}</p>
                    <p className="text-alpine-slate dark:text-gray-500 text-xs">{review.location}</p>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <div className="container-narrow relative z-10 text-center">
          <MotionWrapper variant="scale">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link
                href="/contact"
                className="btn-primary bg-white text-forest-green hover:bg-gray-100 btn-glow btn-magnetic"
              >
                Contact Us Today
              </Link>
              <Link
      ...
      <section className="section-padding bg-luxury-light dark:bg-gray-900">
        <div className="container-bvp">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {/* Hayfork Office */}
            <MotionWrapper variant="fadeLeft" delay={0.1}>
              <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-luxury dark:shadow-none border border-gray-100 dark:border-gray-700">
                ...
                <div className="p-6">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-forest-green/10 dark:bg-forest-green/20 flex items-center justify-center shrink-0">
                      <svg className="w-6 h-6 text-forest-green dark:text-river-stone" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                    </div>
                    <div>
                      <h3 className="font-heading text-h4 text-charcoal-ink dark:text-white mb-1">Hayfork Office</h3>
                      <p className="text-cabin-timber dark:text-gray-400 text-sm">7050 State Hwy 3, PO Box 970</p>
                      <p className="text-cabin-timber dark:text-gray-400 text-sm mb-2">Hayfork, CA 96041</p>
                      <a href="tel:5306285850" className="text-forest-green dark:text-river-stone font-medium text-sm hover:text-deep-pine transition-colors inline-flex items-center gap-1.5">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-luxury dark:shadow-none border border-gray-100 dark:border-gray-700">
                ...
                <div className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="font-heading text-h4 text-charcoal-ink dark:text-white mb-1">Weaverville Office</h3>
                      <p className="text-cabin-timber dark:text-gray-400 text-sm">1313 S Main St, Suite A, PO Box 1263</p>
                      <p className="text-cabin-timber dark:text-gray-400 text-sm mb-2">Weaverville, CA 96093</p>
                      <a href="tel:5306235690" className="text-forest-green dark:text-river-stone font-medium text-sm hover:text-deep-pine transition-colors inline-flex items-center gap-1.5">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                        (530) 623-5690
                      </a>
